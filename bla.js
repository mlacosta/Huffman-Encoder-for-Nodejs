const constants = require('./src/_old/lookup');

const lib = require('./src/_old/createEncoder');

const dataset1 = `[SERVICE]
    Flush     1
    Daemon    off
    Log_Level info
    HTTP_Server On
...
[SERVICE]
    ...
    [OUTPUT]
    Name  stdout
    Match *", "remote_ip"=>"::ffff:10.123.144.18"
    [INPUT]
    Name        tail
...
const encoder = lib.createEncoder(dataset1);
...
// const urlInBits = [...url].map((val) => lib.dec2bin(constants.tobinary[val]));
// console.log(urlInBits);
const result = lib.decodeConfig(url, encoder);
